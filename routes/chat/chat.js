...
// 对相关数据进行处理，更新用户数据库中的interest和mastery
const updateUserInterestAndMastery = async (content, userid) => {
    try {
        const similarityData = await getEmbedding(content)
        const similarityList = similarityData.filter(i => i.cosine > 0.4)
        const top3 = similarityList.slice(0, 3)

        const similarityTitle = top3
            .filter(i => i.cosine > 0.5)
            .map(i => i.id)
...
router.post('/getClientId', async (req, res) => {
    try {
        ...
        console.error('/getClientId 出错：', error)
        res.status(400).json({ msg: '失败', error })
    }
})

// 创建一个promise去确保similarityList有值
const createPromise = () => {
...
router.get('/connetSSE', async (req, res) => {
    ...
    // 创建一个promise
    const { createP, resolveFn } = createPromise()
